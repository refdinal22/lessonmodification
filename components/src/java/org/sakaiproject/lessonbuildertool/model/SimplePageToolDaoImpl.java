...
public class SimplePageToolDaoImpl extends HibernateDaoSupport implements SimplePageToolDao {
	...
	public String getExample(){
		DetachedCriteria d = DetachedCriteria.forClass(SimplePageExample.class).add(Restrictions.eq("id", 2));
		// DetachedCriteria d = DetachedCriteria.forClass(SimplePageExample.class);
		List l = getHibernateTemplate().findByCriteria(d);

		if (l != null && l.size() > 0) {
			return ( (SimplePageExample) l.get(0)).getText();
		} else {
			return "error";
		}
	}

	public List<SimplePageExample> getAllExample(){
		DetachedCriteria d = DetachedCriteria.forClass(SimplePageExample.class);
		// DetachedCriteria d = DetachedCriteria.forClass(SimplePageExample.class);
		List<SimplePageExample> l = (List<SimplePageExample>) getHibernateTemplate().findByCriteria(d);
	...
	public List<SimplePageGlossary> getAllGlossary(){
		DetachedCriteria d = DetachedCriteria.forClass(SimplePageGlossary.class);
		// DetachedCriteria d = DetachedCriteria.forClass(SimplePageExample.class);
