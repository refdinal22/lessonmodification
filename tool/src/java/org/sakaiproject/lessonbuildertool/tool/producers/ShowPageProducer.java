...
public class ShowPageProducer implements ViewComponentProducer, DefaultView, NavigationCaseReporter, ViewParamsReporter {
	...
	private void createToolBar(UIContainer tofill, SimplePage currentPage) {
		...
		//modifikasi
		// UIComponent examplelink = UIInternalLink.makeURL(tofill, "example-link", "#");
		// examplelink.decorate(new UITooltipDecorator(messageLocator.getMessage("simplepage.example")));
		//modifikasi
	...
	// modifikasi
	// private void createExampleDialog(UIContainer tofill, SimplePage currentPage) {
	// 	UIOutput.make(tofill, "example-dialog").decorate(new UIFreeAttributeDecorator("title", messageLocator.getMessage("simplepage.example")));
	// 	UIForm form = UIForm.make(tofill, "example-form");
	// 	// makeCsrf(form, "csrf7");

	// 	// UIOutput.make(form, "subpage-label", messageLocator.getMessage("simplepage.pageTitle_label"));
	// 	UIInput.make(form, "example-name", "#{simplePageBean.nameE}");

	// 	UICommand.make(form, "create-example", messageLocator.getMessage("simplepage.create"), "#{simplePageBean.processActionSubmit}");
	// 	UICommand.make(form, "cancel-example", messageLocator.getMessage("simplepage.cancel"), null);
